...
const EditProductModal: React.FC<EditProductModalProps> = ({ product, onClose, onSave }) => {
  const [formData, setFormData] = useState({
    name: product.name,
    minPrice: product.priceRange?.min.toString() || product.price.toString(),
    maxPrice: product.priceRange?.max.toString() || '',
    category: product.category,
    subcategory: product.subcategory,
    description: product.description,
    composition: product.composition,
    fabricPattern: product.fabricPattern || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedProduct: Product = {
        ...
        name: formData.name,
        price: minPrice,
        priceRange: formData.maxPrice ? { min: minPrice, max: maxPrice } : undefined,
        category: formData.category as ProductCategory,
        subcategory: formData.subcategory,
        description: formData.description,
        composition: formData.composition,
        fabricPattern: formData.category === 'fabric' ? formData.fabricPattern : undefined,
        images: imageUrls,
        ...
        >
          <div className="p-6">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-serif">Edit Product - {product.id}</h2>
              <button
                onClick={onClose}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label htmlFor="maxPrice" className="block text-sm font-medium text-gray-700 mb-1">
                    Maximum Price (₹) <span className="text-gray-500">(Optional)</span>
                  </label>
                  <input
                    type="number"
                    id="maxPrice"
